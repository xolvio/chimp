...
function Monkey (options) {
  ...
  //  log.debug('[cuke-monkey][monkey] SIGINT detected, killing process');
  //  process.kill();
  //});
...
/**
 * Starts a cuke-monkey server on a freeport or on options.serverPort if provided
 *
 * @api public
 */
Monkey.prototype.server = function () {
  var self = this;
...
Monkey.prototype._startServer = function (port) {
  ...
  log.info('Cuke-Monkey server is running on port', port)

};

Monkey.prototype._setupRoutes = function (server) {
  var self = this;
  server.route({
    method: 'GET',
    path: '/run',
 ...
 * @api public
 */
Monkey.prototype.rerun = function (callback) {
