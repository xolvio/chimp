...
Monkey.prototype.interrupt = function (callback) {
  ...
  if (!self.processes || self.processes.length === 0) {
    log.debug('[cuke-monkey] no processes to interrupt');
    callback();
    return;
  }
  ...
  async.series(processes, function(e, r) {
    log.debug('[cuke-monkey] Finished interrupting processes', e, r);
...
Monkey.prototype.rerun = function () {
  ...
  log.debug('[cuke-monkey] rerun()');

  var self = this;

  if (self.isRerunning) {
    log.debug('[cuke-monkey] interrupt in progress, ignoring rerun');
    return;
  }
  self.isRerunning = true;

  self.run(function () {
    log.debug('[cuke-monkey] finished run in rerun()');
