...
SessionManager.prototype.webdriver = require('./webdriverio-with-sync');
...
SessionManager.prototype._monkeyPatchBrowserSessionManagement = function (browser, sessions) {
  ...
  if (global.init === browser._init) {
    global.init = browser.init;
  }
  if (global.initSync === browser._initSync) {
    global.initSync = browser.initSync;
  }
  if (global.initAsync === browser._initAsync) {
    global.initAsync = browser.initAsync;
  }

  browser.end = callbacker.bind(browser);
  browser.endSync = browser.end;
  browser.endAsync = browser.end
  if (global.end === browser._end) {
    global.end = browser.end;
  }
  if (global.endSync === browser._endSync) {
    global.endSync = browser.endSync;
  }
  if (global.endAsync === browser._endAsync) {
    global.endAsync = browser.endAsync;
  }

  browser.endAll = callbacker.bind(browser);
  browser.endAllSync = browser.endAll;
  browser.endAllAsync = browser.endAll;
  if (global.endAll === browser._endAll) {
    global.endAll = browser.endAll;
  }
  if (global.endAllSync === browser._endAllSync) {
    global.endAllSync = browser.endAllSync;
  }
  if (global.endAllAsync === browser._endAllAsync) {
    global.endAllAsync = browser.endAllAsync;
  }
...
SessionManager.prototype.killCurrentSession = function (callback) {
  ...
  var wdHubSession = 'http://localhost:' + this.options.port + '/wd/hub/session';
