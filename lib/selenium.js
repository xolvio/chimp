...
function Selenium (options) {
  ...
  if (!this.options['clean-selenium-server']) {
    ...
    if (typeof Selenium.instance === 'object') {
      log.debug('[cuke-monkey][selenium]', 'Selenium object already exists, not creating a new one');
      return Selenium.instance;
    }
    log.debug('[cuke-monkey][selenium]', 'Selenium object created');
...
Selenium.prototype.start = function (callback) {
  ...
  log.debug('[cuke-monkey][selenium]', 'Start');

  if (self.child) {
    log.debug('[cuke-monkey][selenium]', 'Already running, doing nothing');
    callback(null);
    return;
  }
  ...
  self.install(function (error) {
    ...
    log.debug('\n[cuke-monkey][selenium] hub can be seen at http://localhost:' + port + '/wd/hub');
...
Selenium.prototype.stop = function (callback) {
  ...
  log.debug('[cuke-monkey][selenium]', 'Stopping');
