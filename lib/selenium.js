...
function Selenium (options) {
  ...
  if (!this.options['clean-selenium-server']) {
    ...
    if (typeof Selenium.instance === 'object') {
      log.debug('[selenium]', 'Selenium object already exists, not creating a new one');
...
Selenium.prototype.start = function (callback) {
  ...
  if (self.child) {
    log.debug('[selenium]', 'Already running, doing nothing');
    callback(null);
    return;
  }
  ...
  self.install(function (error) {
    ...
    log.debug('\n[selenium] hub can be seen at http://localhost:' + port + '/wd/hub');
