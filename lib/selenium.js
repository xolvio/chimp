  ...
  async.waterfall([
    ...
    function (port, callback) {
      ...
      self.install(function (error) {
        ...
        if (isDebugging()) {
          console.log('[selenium] hub can be seen at http://localhost:' + port + '/wd/hub');
        }

        selenium.start(seleniumOptions, function (error, seleniumChild) {

          _seleniumChild = seleniumChild;
          ...
          if (isDebugging()) {
            _seleniumChild.stderr.on('data', function (data) {
              process.stdout.write('[selenium] ' + data.toString());
            });
          }

          // selenium started callback
          callback();
...
Selenium.prototype.interrupt = function (callback) {
  ...
  try {
    process.kill(_seleniumChild.pid);
  } catch (e) {
    return;
  }
  ...
  if (callback) {
    var waitForProcessToDie = setInterval(function () {
      try {
        process.kill(_seleniumChild.pid, 0);
      } catch (e) {
        _seleniumChild = null;
