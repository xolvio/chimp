...
Chimp.prototype.init = function (callback) {
  ...
  log.debug('[chimp] Running npm install');
  this.exec('npm install', function (error, stdout, stderr) {
    log.debug('[chimp] stdout:', stdout);
    log.debug('[chimp] stderr:', stderr);
    if (error !== null) {
      log.error('[chimp] stderr:', stderr);
...
Chimp.prototype.run = function (callback) {
  ...
  async.series([
    ...
    if (self.options.simianAccessToken) {
      ...
      simianReporter.report(res, function (reportErr) {
        callback(reportErr, res);
      });
    } else {
      callback(null, res);
    }
