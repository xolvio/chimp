module.exports = function () {
  ...
  this.registerHandler('BeforeFeatures', function (event) {
    ...
    var setupDdp = function () {
      ...
      if (process.env['chimp.ddp']) {
        ...
        try {
          ...
          log.info('[chimp][hooks] connecting via DDP had no error');
        } catch (error) {
    ...
    // Make all WebDriver.io commands global
    _.forEach(global.browser, function (command, commandName) {
      if (_.isFunction(command)) {
        if (global.hasOwnProperty(commandName)) {
          log.info('[chimp][hooks] Not exposing WebDriver.io command "' + commandName + '" to the global scope' +
            ' because name is already taken');
        } else {
          global[commandName] = command.bind(global.browser);
        }
      }
    });
  });
  ...
  this.StepResult(function (event) {
    ...
    if (process.env['chimp.screenshotsOnError'] !== 'false' && !stepResult.isSuccessful()) {
      ...
      global.browser.captureSync('step_failed');
    }
  });
