    ...
    var initBrowser = Promise.promisify(function (callback) {
      ...
      global.browser.init(function () {
        ...
        global.browser.addCommand('capture', function (name, callback) {
          ...
          log.debug('[cuke-monkey][hooks] saving screenshot to', location);
          return this.saveScreenshot(path.join(process.env['monkey.screenshotPath'], location), false, callback);
        });
        ...
        browser.on('error', function () {
          log.debug('[cuke-monkey][hooks] browser error event - capturing screenshot');
