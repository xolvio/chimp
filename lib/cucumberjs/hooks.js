module.exports = function () {
  ...
  this.registerHandler('BeforeFeatures', function (event, callback) {
    ...
    var initBrowser = Promise.promisify(function (callback) {
      log.debug('[cuke-monkey][hooks] init browser', global.browser);
      global.browser.init(function () {

        log.debug('[cuke-monkey][hooks] init browser callback');

        var self = this;
        var sessionID = self.requestHandler.sessionID;

        this.timeoutsAsyncScript(parseInt(process.env['monkey.timeoutsAsyncScript']));
        ...
        global.browser.addCommand('sauceJobStatus', function (status, done) {
          ...
          sauceAccount.updateJob(self.requestHandler.sessionID, status, done);
        });
