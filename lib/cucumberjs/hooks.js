    ...
    var setupBrowser = function () {
      ...
      if (fs.existsSync(customChimpConfigPath)) {
        ...
        if (global.browser.select) {
          ...
          var syncByDefault = webdriverOptions.sync;

          var _browserA = global.browser.select("browserA");
          var _browserB = global.browser.select("browserB");




          var browserAremoteWrapper = wrapAsyncObject(_browserA, commandNames, {
          ...
          var _browserAaddcommand = _browserA.addCommand;
          _browserA.addCommand = function (fnName, fn, forceOverwrite) {
            var result = _browserAaddcommand.call(_browserA, fnName, Promise.async(fn), forceOverwrite);
            var commandWrapper = wrapAsyncObject(_browserA, [fnName], {
              syncByDefault: syncByDefault,
              wrapAsync: wrapAsyncForWebdriver
            });
            _.defaults(browserAremoteWrapper, commandWrapper);
          ...
          _.forEach([
            ...
            if (_browserA[methodName]) {
              browserAremoteWrapper[methodName] = _browserA[methodName].bind(_browserA);
            }
          });

          global.browser.browserA = browserAremoteWrapper;



        } else {
          console.log("doesn't exist in remoteSession");
