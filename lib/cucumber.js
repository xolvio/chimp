...
Cucumber.prototype.start = function (callback) {
  ...
  log.debug('[cuke-monkey][cucumber] Running');
  ...
  self.cucumberChild.on('close', function (code) {
    log.debug('[cuke-monkey][cucumber] Closed with code', code);
    if (!self.cucumberChild.stopping) {
      callback(code !== 0 ? 'Cucumber steps failed' : null, results);
    }
  });
...
Cucumber.prototype.interrupt = function (callback) {
  ...
  log.debug('[cuke-monkey][cucumber] interrupting cucumber');

  var self = this;

  if (!self.cucumberChild) {
    log.debug('[cuke-monkey][cucumber] no child to interrupt');
    return callback();
  }
